        ...
        <tbody>
          <tr>
            <td style={{ verticalAlign:"top", paddingRight:16 }}>
              <img src={left} alt="left" style={{ width:110, display:"block" }} />
            </td>
            <td style={{ verticalAlign:"middle" }}>
              <div style={{ fontWeight:700, fontSize:16 }}>{name}</div>
              <div style={{ color:"#555", marginTop:4 }}>{title}</div>
              <div style={{ marginTop:8 }}>
                <a href={"mailto:"+email} style={{ color:accent, textDecoration:"none", marginRight:10 }}>{email}</a>
                <span style={{ color:"#ccc", margin:"0 8px" }}>•</span>
                <span style={{ color:accent }}>{phone}</span>
              </div>
              <div style={{ marginTop:10, display:"flex", gap:8 }}>
                {icons.map((src,i)=>(
                  <a key={i} href={links[i]} target="_blank" rel="noreferrer"><img src={src} width="18" alt="icon" /></a>
                ))}
              </div>
            </td>
          </tr>
